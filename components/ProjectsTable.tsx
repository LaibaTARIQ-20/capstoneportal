...
import {
  collection, getDocs, deleteDoc, doc, query, where, writeBatch,
...
export default function ProjectsTable({ isAdmin, supervisorId }: ProjectsTableProps) {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Delete ${selected.size} selected project${selected.size > 1 ? "s" : ""}? This cannot be undone.`)) return;
                ...
                <tr key={project.id}
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => router.push(isAdmin ? `/admin/projects/${project.id}` : `/faculty/projects/${project.id}`)}
