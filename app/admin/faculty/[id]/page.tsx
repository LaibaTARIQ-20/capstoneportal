...
import {
  ...
  ArrowLeft, Mail, Phone, Building2, BadgeCheck, Venus, Mars,
} from "lucide-react";
...
function DesignationBadge({ designation }: { designation: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${colorMap[designation] || "bg-gray-100 text-gray-700"}`}>
      {designation}
    </span>
  );
}
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const projectsQuery = query(collection(db, "projects"), where("supervisorId", "==", facultyId));
        const projectsSnap = await getDocs(projectsQuery);
        setProjects(projectsSnap.docs.map((d) => ({ id: d.id, ...d.data() })) as FacultyProject[]);
      } catch (err) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href="/admin/faculty" className="mt-3 inline-block text-sm text-blue-600 hover:underline">← Back to faculty list</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="fixed left-0 top-0 flex h-full w-60 flex-col bg-zinc-900 px-4 py-6">
        ...
        <nav className="flex flex-col gap-1">
          <Link href="/admin/dashboard" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors">
            <LayoutDashboard size={17} />Dashboard
          </Link>
          <Link href="/admin/projects" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors">
            <FolderOpen size={17} />Projects
          </Link>
          <Link href="/admin/faculty" className="flex items-center gap-3 rounded-lg bg-zinc-800 px-3 py-2.5 text-sm font-medium text-white">
      ...
      <main className="ml-60 flex-1 px-8 py-8">
        <Link href="/admin/faculty" className="mb-6 inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">
          ...
          <div className="lg:col-span-1">
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="mb-6 flex flex-col items-center text-center">
                ...
                <h2 className="mt-4 text-lg font-bold text-gray-900">{faculty.name}</h2>
                <div className="mt-1.5"><DesignationBadge designation={faculty.designation} /></div>
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                <h2 className="text-sm font-semibold text-gray-800">Supervised Projects</h2>
                <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700">{projects.length} projects</span>
              </div>
              {projects.length === 0 && (
                    ...
                    {projects.map((project, index) => (
                      <tr key={project.id} className="transition-colors hover:bg-gray-50">
                        ...
                        <td className="px-6 py-3">
                          {project.sdg
                            ? <span className="rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700">{project.sdg}</span>
                            : <span className="text-gray-400">—</span>}
                        </td>
                        <td className="px-6 py-3 text-gray-500">{project.studentCount} students</td>
